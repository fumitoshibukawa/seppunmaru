...
void Collect(void)
{
	...
	/// 右前の回収処理 ///
	//////////////////////
	for(int step = 0; step < max_search_front; step++){
...
/*			
		...
		traj_tracking( (offset_search * step + ADJUSTMENT_SEARCH_START_POS -3.0f) * 2 * PI / 4, -92.0f, 2.0f);
		//rotate(-90.0f);
...
void catch_ball(float distance, float angle){
	...
	HUG_BULLDOZE;
	sleep( 100 );
	traj_tracking( -distance - 6.5f, 0.0f, 5.0f );
...
void catch_zeitaku(float distance, float angle){
	//接近
	rotate(angle);
	translate(distance -7.0f); // -5.0f追加
	//sleep( 100 );
	//磁石でキャッチ
